...
pub fn get_many_mut<const N: usize>(
    ...
    // First, collect all frame_ids, or return None if any page is missing
    let mut frame_ids = [0usize; N];
    for (i, page_num) in pages.iter().enumerate() {
        frame_ids[i] = *self.pages.get(page_num)?;
    }

    // Check for duplicates in frame_ids to avoid multiple mutable borrows to the same frame
    for i in 0..N {
        for j in (i + 1)..N {
            if frame_ids[i] == frame_ids[j] {
                return None;
            }
        }
    }

    // Now, collect mutable references in a separate step
    // This is safe because we checked for duplicates above
    let buffer = &mut self.buffer;
    // SAFETY: We have checked for duplicates in frame_ids, so all indices are unique.
    let mut refs: [*mut MemPage; N] = [std::ptr::null_mut(); N];
    for (i, &frame_id) in frame_ids.iter().enumerate() {
        let frame = unsafe { &mut *(&mut buffer[frame_id] as *mut Frame) };
        frame.set(REF_FLAG | DIRTY_FLAG);
        refs[i] = &mut frame.page as *mut MemPage;
    }
    // SAFETY: All pointers are unique and valid for the duration of this function.
    Some(unsafe { std::array::from_fn(|i| &mut *refs[i]) })
}
